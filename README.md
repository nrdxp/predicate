...
* **The Trajectory Commit Gate:** Once validation error converges to zero, the agent executes a structured self-audit. It checks the generated diff against architectural simplicity (complecting check), design volatility (Lowy check), style rules, and evaluates the active sketch's `RUBRIC` ledger to confirm all strategic constraints are met. If the changes converged in a single iteration, a skeptical audit check is enforced to ensure no hidden assumptions were left unverified. Under `CONTROL_MODE: AUTOMATIC`, only if this self-audit confirms compliance (quality score = 1.0) does the runner commit and proceed. Otherwise, or if the local optimization fails to converge, the system halts, preserves all unverified changes for inspection, and returns control to manual mode with a detailed trace log.
