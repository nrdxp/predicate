...
> [!NOTE]
> DepMap gives a more comprehensive view of API surfaces than manual searches. Leverage it to prevent dependency API hallucinations.
...
> [!TIP]
> Use plain identifier names without special characters or prefixes.
...
> [!IMPORTANT]
> Always use `dep_map` before calling unfamiliar library functions. Verify the API exists before generating code.
...
### Before Writing Code
1. **New codebase?** $\rightarrow$ `repo_map` first
2. **Using external library?** $\rightarrow$ `dep_map` to verify API
3. **Modifying existing code?** $\rightarrow$ `search_identifiers` to find all usages
