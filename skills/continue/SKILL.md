...
5. **Commit message** — [conventional format](file:///var/home/nrd/git/github.com/nrdxp/predicate/skills/commit-hygiene/SKILL.md) (conforming to the commit-hygiene skill guidelines)
...
4. **Skills** — all `SKILL.md` files under `.agents/skills/` (foundational axioms like `constitution` and `engineering`, plus active language/domain skills)

> [!IMPORTANT]
> **Self-diagnose drift proactively.** If you find yourself uncertain about what constraints apply, unable to assess whether your work aligns with project principles, or producing JUSTIFICATION blocks without confident alignment reasoning — that is a signal to re-read workspace skills _now_, not after the next commit. The cost of a re-read is negligible; the cost of drifting is cumulative and silent. Invoke `/predicate` for a full refresh, or selectively re-read the specific skills you've lost context on.
