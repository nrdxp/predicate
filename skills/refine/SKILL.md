...
- **Sieving & Cutting Pruning Invariant:** Counter the additive bias of self-correction loops (where models continuously add helper variables, log structures, and complex logic to patch warnings). Cutting is a core mechanism of convergence. The agent MUST actively evaluate and prune superfluous structures, redundant variables, comments, or dependencies introduced in follow-up iterations that no longer serve the target goal, folding them into the minimal representation. Do not prune pre-existing public API endpoints or parameter interfaces unless explicitly requested by `CTX.GOAL` or verified as unused.
