...
- **Hostile Maintainer Review:** Mandates submitting the completed changeset to a panel of independent, critical maintainer subagents representing codebase owners who critique design complexity, module boundaries, and documentation. The review operates under the **Refine/Review Dialectic**: both the refiner and reviewers are instructed as world-class experts with opposite methods but a shared goal of a compelling codebase improvement. The refiner must take a stand to defend its macro-architectural design against myopic reviews while avoiding the resubmission of deferred bias (supported by meticulous note-keeping), while maintainers reject lazy hacks/BS code but constructively solve problems, conceding with humility to superior work. All comments in `REVIEW_LEDGER` must be resolved (via commits or justified rebuttals) and marked approved. API stability compromises override nits only after stability is reached.
