...
#
# <law-dir>       ledger/contracts — where entry.ncl, entry_apply.ncl, and
#                 entries_query.ncl live.
...
# entries_query.ncl paths: <out-dir>'s composed copies when a registry file
# was given and exists, <law-dir>'s own files (importing the plugin's empty
# default) otherwise.
compose_tag_registry() {
  ...
  if [ -n "$registry_file" ] && [ -f "$registry_file" ]; then
    ...
    COMPOSED_QUERY="$out_dir/entries_query.ncl"
  else
    COMPOSED_APPLY="$law_dir/entry_apply.ncl"
    COMPOSED_QUERY="$law_dir/entries_query.ncl"
  fi
}
